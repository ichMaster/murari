...
- `/release-version 0.1.0 Install brainstormer agent; Prove the loop by hand` -- bump with provided changelog items
...
Version notation `A.B.C`: `A` = 0 during the prototype (→ 1 at graduation, once the agent is proven / v0.5 accepted), `B` = roadmap phase (v0.0–v0.5, from [spec/roadmap.md](../../../spec/roadmap.md)), `C` = post-phase fix on that phase. So roadmap phase `v0.B` → semver `0.B.0`; a fix after it bumps `C` (e.g. v0.1 → `0.1.0`, a follow-up fix → `0.1.1`). Releases are cut per phase; **v0.0 ships no release, so the first tag is `0.1.0`**. **Never change the version without explicit user confirmation.**
...
3. Summarize the changes into concise bullet points (group related commits; reference the roadmap phase `pN` where relevant)
...
- **Don't modify source files.** This skill only touches version metadata (VERSION, README.md, RELEASE.txt), never the `agent`, `chat`, `tui`, `orchestrator`, or `sandbox` code, and never the `spec/` documents.
