...
| `murari chat [<session-dir>] [--new "<тема>"] [--name S] [--style K]` | **Facilitated chat (v0.2)** over a session — the headless stand-in for the v0.3 TUI. A reply classified into a role (Фантазер/Дослідник/Опонент/Алхімік, or a Суддя/Ткач order) is recorded as **your** move (free, `born_from: user`) and the complementary agent move is planned (adversarial only in `debate` — no winner). **Runs launch only via `/go [стиль] [глибина]`** over the session topic; bare `/go` runs the planned move. Otherwise the Ведучий converses over DOCUMENT.md (summaries, discussion) and may itself trigger at most a single tiny role move. Commands: `/style [key]` · `/go [стиль] [глибина]` · `/ledger` · `/quit` (the session dir remains). |
